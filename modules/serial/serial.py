...
from PySide6.QtCore import QObject, Property, Signal, Slot, QTimer, QThread, QMutex, QMutexLocker, QLoggingCategory
from PySide6.QtSerialPort import QSerialPort, QSerialPortInfo
from PySide6.QtWidgets import QApplication
from PySide6.QtQml import QQmlApplicationEngine
from PySide6.QtCore import QUrl

MODULE_WINDOW_TITLE = "串口调试助手"
def register_types():
...
class SerialHandler(QObject):
    ...
    def _on_error(self, error):
        ...
        if error != QSerialPort.SerialPortError.NoError:
            error_msg = f"串错误: {self._serial.errorString()}"
            self.errorOccurred.emit(error_msg)
            print(error_msg)
...
if __name__ == "__main__":
    ...
    # 获取当前模块信息
    MODULE_NAME = os.path.basename(os.path.dirname(__file__))  # 获取模块名称
    MODULE_TITLE = MODULE_WINDOW_TITLE  # 这个可以根据需要修改
    ...
    main_window_path = os.path.join(project_root, "MainWindow.qml")
    module_qml_path = os.path.join(current_dir, f"{MODULE_NAME.capitalize()}.qml")
    
    # 添加标准QML导入路径
    engine.addImportPath(project_root)
    engine.addImportPath(current_dir)
    
    # 设置标准上下文属性
    context = engine.rootContext()
    context.setContextProperty("moduleTitle", MODULE_TITLE)
    context.setContextProperty("modulePath", QUrl.fromLocalFile(module_qml_path))
    
    # 加载主窗口
    engine.load(QUrl.fromLocalFile(main_window_path))
    
    if not engine.rootObjects():
        print(f"{MODULE_NAME} 模块QML加载失败")
        sys.exit(-1)
    
    # 设置标准窗口属性
    window = engine.rootObjects()[0]
    window.setProperty("moduleTitle", MODULE_TITLE)
    window.setProperty("modulePath", QUrl.fromLocalFile(module_qml_path))
